 ...
 */
export function buildBasicBlocks(ilFunction: ILFunction): BasicBlock[] {
 ...
 */
function findBasicBlockLeaders(instructions: readonly ILInstruction[]): number[] {
 ...
 */
function extractBranchTarget(instruction: ILInstruction): number | null {
  if (instruction.operands.length > 0) {
    const target = instruction.operands[instruction.operands.length - 1] as ILValue;
 ...
 */
export function buildControlFlowEdges(blocks: BasicBlock[]): CFGEdge[] {
  const edges: CFGEdge[] = [];
 ...
 */
function findBlockContainingInstruction(blocks: BasicBlock[], instructionIndex: number): number {
  let currentIndex = 0;
 ...
 */
export function updateBlockConnections(blocks: BasicBlock[], edges: CFGEdge[]): void {
...
export function computeImmediateDominators(cfg: ControlFlowGraph): Map<number, number> {
  ...
  const immediateDominators = new Map<number, number>();
  ...
  while (changed) {
    ...
    for (const blockId of blocks) {
      if (blockId === entryBlock) continue;

      const block = cfg.blocks.get(blockId);
      if (!block || block.predecessors.size === 0) continue;
 ...
 */
function getDominatorPath(blockId: number, dominators: Map<number, number>): number[] {
  const path: number[] = [];
  let current = blockId;
  ...
  while (current !== undefined) {
    ...
    if (dominator === current) break; // Reached entry block
    current = dominator!;
  }
 ...
 */
export function buildDominanceTree(
  ...
  for (const [block, idom] of immediateDominators) {
    if (!children.has(idom)) {
      children.set(idom, new Set<number>());
    }
    if (!children.has(block)) {
      children.set(block, new Set<number>());
    }
  }
  ...
  function computeDepths(blockId: number, currentDepth: number): void {
 ...
 */
export function findBackEdges(cfg: ControlFlowGraph): BackEdge[] {
 ...
 */
export function identifyNaturalLoops(
  cfg: ControlFlowGraph,
  backEdges: BackEdge[],
  dominanceTree: DominanceTree
): NaturalLoop[] {
  const naturalLoops: NaturalLoop[] = [];
 ...
 */
function blockDominates(blockA: number, blockB: number, dominanceTree: DominanceTree): boolean {
  let current = blockB;
 ...
 */
function findLoopBody(cfg: ControlFlowGraph, backEdge: BackEdge): Set<number> {
 ...
 */
function findLoopExits(cfg: ControlFlowGraph, loopBody: Set<number>): Set<number> {
 ...
 */
function calculateNestingDepth(header: number, existingLoops: NaturalLoop[]): number {
  let maxDepth = 0;
 ...
 */
function analyzeLoopCharacteristics(cfg: ControlFlowGraph, loopBody: Set<number>) {
    ...
    for (const instruction of block.instructions) {
      switch (instruction.type) {
        ...
        case ILInstructionType.STORE_VARIABLE:
          ...
          if (instruction.operands.length > 0) {
            const operand = instruction.operands[0] as ILValue;
 ...
 */
function updateInnermostFlags(loops: NaturalLoop[]): void {
  for (const loop of loops) {
    ...
    (loop as any).isInnermost = isInnermost;
  }
}
 ...
 */
export function extractVariableUses(
  instruction: ILInstruction,
  blockId: number,
  instructionIndex: number
): VariableUse[] {
  ...
  for (let i = startIndex; i < instruction.operands.length; i++) {
    const operand = instruction.operands[i] as ILValue;
 ...
 */
function classifyDefinitionType(instruction: ILInstruction) {
  switch (instruction.type) {
 ...
 */
function classifyUsageType(instruction: ILInstruction, operandIndex: number) {
  switch (instruction.type) {
    case ILInstructionType.BRANCH_IF_TRUE:
    case ILInstructionType.BRANCH_IF_FALSE:
      return 'condition';
 ...
 */
function isDefinitionInstruction(instruction: ILInstruction): boolean {
 ...
 */
export function measureAnalysisPerformance<T>(
  analysisFunction: () => T,
  description: string
): { result: T; metrics: Partial<CFGAnalysisMetrics> } {
 ...
 */
export function validateCFGStructure(cfg: ControlFlowGraph): string[] {
  const errors: string[] = [];
 ...
 */
export function createInstructionLocation(
  blockId: number,
  instructionIndex: number,
  instruction: ILInstruction
): InstructionLocation {
