...
export class ControlFlowAnalyzer {
  private readonly options: CFGAnalysisOptions;
  private analysisMetrics: Partial<Record<keyof CFGAnalysisMetrics, number>> = {};
   ...
   */
  public analyzeFunction(ilFunction: ILFunction): ControlFlowAnalysisResult {
    const { result: cfg, metrics: cfgMetrics } = measureAnalysisPerformance(
      () => this.buildControlFlowGraph(ilFunction),
      'CFG Construction'
    );
    ...
    const finalMetrics: CFGAnalysisMetrics = {
   ...
   */
  private buildControlFlowGraph(ilFunction: ILFunction): ControlFlowGraph {
    ...
    // Create blocks map
    const blocksMap = new Map<number, BasicBlock>();
   ...
   */
  private createEmptyDominanceAnalysis(): DominanceAnalysis {
    return {
  ...
  private createEmptyDataDependencyAnalysis(): DataDependencyAnalysis {
    return {
  ...
  private createEmptyLiveVariableAnalysis(): LiveVariableAnalysis {
    return {
      liveIn: new Map(),
      liveOut: new Map(),
      liveRanges: new Map(),
  ...
  private createEmptyCriticalPathAnalysis(): CriticalPathAnalysis {
    return {
      criticalPaths: [],
      hotBlocks: [],
   ...
   */
  private countVariables(cfg: ControlFlowGraph): number {
...
export function analyzeCFG(
  ...
  options: Partial<CFGAnalysisOptions> = {}
): ControlFlowAnalysisResult {
