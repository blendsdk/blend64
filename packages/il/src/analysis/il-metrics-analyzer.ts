...
import { ILFunction, ILInstructionType, ILValue } from '../il-types.js';
...
export class ILMetricsAnalyzer {
  private readonly options: QualityAnalysisOptions;
  ...
  public analyzeFunction(
    ...
    try {
      ...
      const summary = this.buildQualityAnalysisSummary(
        complexityMetrics,
        performancePrediction,
        optimizationReadiness,
        qualityGates,
      ...
      const finalAnalysisMetrics: QualityAnalysisMetrics = {
  ...
  private performComplexityAnalysis(
    ...
    const overallComplexityScore = this.calculateOverallComplexityScore(
      cyclomaticComplexity,
      instructionComplexity.score,
      controlFlowComplexity.score,
      dataFlowComplexity.score
    );
   ...
   * Analyze instruction-level complexity
   */
  private analyzeInstructionComplexity(ilFunction: ILFunction): InstructionComplexityScore {
    ...
    for (const instruction of ilFunction.instructions) {
      ...
      instruction.operands.forEach(operand => {
        const value = operand as ILValue;
   ...
   */
  private isComplexInstruction(type: ILInstructionType): boolean {
    return [
  ...
  private isBranchInstruction(type: ILInstructionType): boolean {
    return [
  ...
  private isMemoryOperation(type: ILInstructionType): boolean {
    return [
  ...
  private isArithmeticOperation(type: ILInstructionType): boolean {
    return [
  ...
  private isTemporaryVariable(name: string): boolean {
  ...
  private extractCycleEstimates(sixtyTwo6502Analysis: SixtyTwo6502ValidationResult): CycleEstimate {
    return {
  ...
  private determineOverallGateStatus(gates: QualityGate[]): QualityGateStatus {
  ...
  private getDefaultQualityThresholds(): QualityThresholdSet {
    return {
      maxComplexity: 60,
  ...
  private estimatePredictionAccuracy(performancePrediction: PerformancePredictionModel): number {
    return performancePrediction.estimatedCycles.confidence;
  }

  private calculateAnalysisCompleteness(): number {
  ...
  private createEmptyComplexityMetrics(): ILComplexityMetrics {
    return {
  ...
  private createEmptyPerformancePrediction(): PerformancePredictionModel {
    return {
      estimatedCycles: {
        bestCase: 0,
        averageCase: 0,
  ...
  private createEmptyOptimizationReadiness(): OptimizationReadinessAnalysis {
    return {
  ...
  private createEmptyQualityGates(): QualityGateAnalysis {
    return {
      gates: [],
      overallStatus: 'pass',
      qualityScore: 100,
 ...
 * @returns Complete quality metrics analysis result
 */
export function analyzeILQuality(
  ilFunction: ILFunction,
  cfgAnalysis: ControlFlowAnalysisResult,
  sixtyTwo6502Analysis: SixtyTwo6502ValidationResult,
  options: Partial<QualityAnalysisOptions> = {}
): ILQualityAnalysisResult {
