...
describe('ILAnalyticsSuite', () => {
  let analyticsSuite: ILAnalyticsSuite;
  ...
  describe('Program Analysis', () => {
    ...
    it('should classify program complexity correctly', async () => {
      ...
      expect(simpleResult.performanceMetrics.totalAnalysisTime).toBeLessThanOrEqual(
        ILAnalyticsSuite.BENCHMARKS.simpleFunctionTarget
      );
      ...
      expect(moderateResult.performanceMetrics.totalAnalysisTime).toBeLessThanOrEqual(
        ILAnalyticsSuite.BENCHMARKS.moderateComplexityTarget
      );
    });
  });
  ...
  describe('Memory Efficiency', () => {
    ...
    it('should meet memory efficiency targets', async () => {
      ...
      expect(result.performanceMetrics.memoryUsage.peakMemoryMB).toBeLessThanOrEqual(
        ILAnalyticsSuite.BENCHMARKS.memoryUsageTargets.simple
      );
    });
  });
  ...
  describe('Edge Cases and Error Handling', () => {
    ...
    it('should handle programs with empty modules', async () => {
      const emptyModuleProgram: ILProgram = {
 ...
 */
function createTestFunction(name: string, instructions: ILInstruction[]): ILFunction {
  return {
    name,
 ...
 */
function createTestVariable(name: string, type: string = 'byte'): any {
  return createILVariable(name, createPrimitiveType(type as any));
}

/**
 * Create test IL constant
 */
function createTestConstant(value: number, type: string = 'byte'): any {
  return createILConstant(createPrimitiveType(type as any), value);
}
