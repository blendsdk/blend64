...
describe('ILOptimizationFramework', () => {
  let framework: ILOptimizationFramework;
  let patternRegistry: any;
  let testProgram: ILProgram;
  let testFunction: ILFunction;
  ...
  beforeEach(() => {
    ...
    testFunction.instructions = [
      createILInstruction(
        ...
        [createILConstant({ kind: 'primitive', name: 'byte' }, 5)],
        1
      ),
      createILInstruction(
        ILInstructionType.LOAD_IMMEDIATE,
  ...
  describe('Function Optimization', () => {
    ...
    it('should handle functions with no instructions', async () => {
      const emptyFunction = createILFunction(
        'emptyFunction',
        ['Test'],
        { kind: 'primitive', name: 'void' },
        { line: 1, column: 1, offset: 0 }
    ...
    it('should optimize functions with different complexities', async () => {
      // Simple function
      const simpleFunction = createILFunction(
        'simple',
        ['Test'],
  ...
  describe('Optimization Levels', () => {
    it('should apply different optimizations based on level', async () => {
      ...
      const results = await Promise.all(
        configs.map(config => framework.optimizeProgram(testProgram, config))
      );
  ...
  describe('Error Handling', () => {
    it('should handle optimization failures gracefully', async () => {
      ...
      const problematicFunction = createILFunction(
        'problematic',
        ['Problematic'],
        { kind: 'primitive', name: 'void' },
        { line: 1, column: 1, offset: 0 }
  ...
  describe('Performance Validation', () => {
    ...
    it('should handle large programs efficiently', async () => {
      ...
      const largeFunction = createILFunction(
        'largeFunction',
        ['Large'],
        { kind: 'primitive', name: 'void' },
        { line: 1, column: 1, offset: 0 }
  ...
  describe('Debug Features', () => {
    it('should generate debug information when enabled', async () => {
      const debugConfig: OptimizationConfig = {
        ...DEFAULT_OPTIMIZATION_CONFIG,
        debug: {
          logDecisions: true,
          generateReports: true,
    ...
    it('should not generate debug information when disabled', async () => {
      const noDebugConfig: OptimizationConfig = {
        ...DEFAULT_OPTIMIZATION_CONFIG,
        debug: {
          logDecisions: false,
  ...
  describe('Complex Optimization Scenarios', () => {
    ...
    it('should handle functions with callback patterns', async () => {
      const callbackFunction = createILFunction(
        'callbackFunction',
        ['Callback'],
        { kind: 'primitive', name: 'void' },
        { line: 1, column: 1, offset: 0 }
