...
export class CompilationUnit {
   ...
   */
  static async fromFiles(filePaths: string[]): Promise<CompilationResult<CompilationUnit>> {
   ...
   */
  async lexAllFiles(): Promise<CompilationResult<void>> {
    ...
    for (const [path, sourceFile] of this.sourceFiles) {
      ...
      try {
        ...
        const compilationError = createCompilationError(
          'LexError',
          `Lexing error: ${error instanceof Error ? error.message : String(error)}`,
          path
        );
   ...
   */
  async parseAllFiles(): Promise<CompilationResult<void>> {
    const errors: CompilationError[] = [];
    const warnings: CompilationError[] = [];
    ...
    for (const [path, sourceFile] of this.sourceFiles) {
      ...
      if (!sourceFile.tokens) {
        const error = createCompilationError(
          'ParseError',
          'Cannot parse file: no tokens available (lexing may have failed)',
          path
        );
        errors.push(error);
   ...
   */
  async lexAndParseAll(): Promise<CompilationResult<void>> {
    // First, lex all files
