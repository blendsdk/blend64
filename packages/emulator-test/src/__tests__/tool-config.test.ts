...
describe('Tool Configuration', () => {
  ...
  describe('getValidatedToolPaths', () => {
    it('should throw error when tools are not available', async () => {
      ...
      await expect(getValidatedToolPaths()).rejects.toThrow(
    ...
    it('should throw error when paths are not configured', async () => {
      ...
      await expect(getValidatedToolPaths()).rejects.toThrow(
        'VICE64_PATH not configured in .env file'
      );
    });
  });
  ...
  describe('Error Messages', () => {
    it('should provide helpful error message for missing VICE64_PATH', () => {
      ...
      try {
        ...
        expect((error as Error).message).toContain('VICE64_PATH not configured in .env file');
        expect((error as Error).message).toContain('Please add: VICE64_PATH=/path/to/vice64');
      }
    });
    ...
    it('should provide helpful error message for missing ACME_PATH', () => {
      ...
      try {
        ...
        expect((error as Error).message).toContain('ACME_PATH not configured in .env file');
        expect((error as Error).message).toContain('Please add: ACME_PATH=/path/to/acme');
      }
    });
  });
});
