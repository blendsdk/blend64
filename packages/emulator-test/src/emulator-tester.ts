...
export class EmulatorTester {
  private acme: AcmeAssembler;
  private vice: ViceEmulator;
   ...
   */
  static async create(): Promise<EmulatorTester> {
   ...
   */
  async runTestCase(testCase: EmulatorTestCase): Promise<EmulatorTestResult> {
    const errors: string[] = [];
   ...
   */
  async runTestSuite(testSuite: EmulatorTestSuite): Promise<EmulatorTestSuiteResult> {
    const startTime = Date.now();
    const results: EmulatorTestResult[] = [];
  ...
  private validateMemoryStates(
    ...
    viceResult: any
  ): { passed: boolean; failures: Array<{ address: number; expected: number; actual: number }> } {
    const failures: Array<{ address: number; expected: number; actual: number }> = [];
    ...
    for (const expected of testCase.expectedMemoryStates) {
      const actual = viceResult.memoryDumps.find((d: any) => d.address === expected.address);
      if (!actual) {
        failures.push({
  ...
  private validateCycleCount(
    testCase: EmulatorTestCase,
    viceResult: any
  ): { passed: boolean; expected?: number; actual?: number; tolerance?: number } {
  ...
  private collectErrors(
    ...
    if (!cycleValidation.passed) {
      errors.push(
        `Cycle count validation failed: expected ${cycleValidation.expected}, ` +
   ...
   */
  private generateOutputPath(assemblyFile: string): string {
   ...
   */
  async getToolVersions(): Promise<{ acme: string; vice: string }> {
