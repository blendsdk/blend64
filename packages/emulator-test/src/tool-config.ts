...
export function getToolPaths(): ToolPaths {
  ...
  if (!acme) {
    throw new Error(
      'ACME_PATH not configured in .env file. ' +
 ...
 */
export async function validateToolPaths(toolPaths: ToolPaths): Promise<boolean> {
  ...
  try {
    ...
    console.error('Tool validation failed:', (error as Error).message);
    return false;
  }
}
 ...
 */
export async function getValidatedToolPaths(): Promise<ToolPaths> {
  const toolPaths = getToolPaths();
  const isValid = await validateToolPaths(toolPaths);

  if (!isValid) {
    throw new Error(
      'Required tools are not available. Please ensure VICE and ACME are properly installed and configured in .env'
    );
  }
