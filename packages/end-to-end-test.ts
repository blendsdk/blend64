...
import { ASTToILTransformer, ILOptimizationFramework, createDefaultPatternRegistry } from './packages/il/src/index.js';
...
async function testCompletePipeline() {
  ...
  try {
    ...
    const sourceCode = readFileSync(join(process.cwd(), 'examples/v02-complete-game-example.blend'), 'utf8');
    ...
    console.log(`   📊 Literals: ${tokens.data.filter(t => t.type === 'NUMBER' || t.type === 'STRING').length}\n`);
    ...
    console.log(`   📊 Functions: ${ast.data.body.filter(d => d.kind === 'FunctionDeclaration').length}`);
    console.log(`   📊 Enums: ${ast.data.body.filter(d => d.kind === 'EnumDeclaration').length}`);
    console.log(`   📊 Variables: ${ast.data.body.filter(d => d.kind === 'VariableDeclaration').length}\n`);
    ...
    if (!ilResult.success) {
      throw new Error(`IL transformation failed: ${ilResult.errors.map(e => e.message).join(', ')}`);
    }

    console.log(`   ✅ IL transformation completed`);
    console.log(`   📊 Modules: ${ilResult.data.modules.length}`);
    console.log(`   📊 Functions: ${ilResult.data.modules.reduce((total, m) => total + m.functions.length, 0)}`);
    console.log(`   📊 Instructions: ${ilResult.data.modules.reduce((total, m) =>
      total + m.functions.reduce((ftotal, f) => ftotal + f.instructions.length, 0), 0)}\n`);
    ...
    return {
      ...
      ilInstructions: ilResult.data.modules.reduce((total, m) =>
        total + m.functions.reduce((ftotal, f) => ftotal + f.instructions.length, 0), 0),
